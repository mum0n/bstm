...
2.  **Composition (∘)**: Represents the functional composition of two components, where one component modulates the parameters of another. This is a powerful tool for creating non-stationary models.
...
*   **State-Space Evolution**: The pipe operator defines a state-space model where one component evolves over the structure of another. This supports both discrete-time dynamics (e.g., `random(s_idx, structure=:spatial, model=icar) |> random(t_idx, structure=:temporal, model=ar1)`) and the creation of spatially-varying curves (e.g., `random(s_idx, structure=:spatial, model=icar) |> random(t_idx, structure=:smooth, model=pspline)`), where the coefficients of the temporal basis functions are modeled as spatial fields.
...
@bstm(
    likelihood(y_hq) ~ intercept() + 
      random(s_idx, model=icar) + 
        nested(proxy_signal, formula="likelihood(y_lq, family=poisson) ~ intercept() + random(x, model=pspline)", data_source=low_quality_data),
    high_quality_data,
...
### 8.4. `random()` Module (Temporal and Seasonal Domains)
...
### 8.5. `random()` Module (Smooth Domain)
...
Spatiotemporal interactions are specified using either the Kronecker product operator (`⊗`) or the `random()` module with `domain=:spacetime`. The `bstm` framework supports the four canonical interaction types defined by Knorr-Held (2000), which are automatically inferred based on the structure of the component models.

| Type         | Description                                    | `random()` Syntax                                             | `⊗` Operator Syntax                                    |
| :-------------| :-----------------------------------------------| :--------------------------------------------------------------| :-------------------------------------------------------|
| **Type I**   | Unstructured (IID) in both space and time.     | `random(s_idx, t_idx, domain=:spacetime, model=(iid, iid))`   | `random(s_idx, model=iid) ⊗ random(t_idx, model=iid)`   |
| **Type II**  | Spatially unstructured, temporally structured. | `random(s_idx, t_idx, domain=:spacetime, model=(iid, ar1))`   | `random(s_idx, model=iid) ⊗ random(t_idx, model=ar1)`   |
| **Type III** | Spatially structured, temporally unstructured. | `random(s_idx, t_idx, domain=:spacetime, model=(besag, iid))` | `random(s_idx, model=besag) ⊗ random(t_idx, model=iid)` |
| **Type IV**  | Fully structured in both space and time.       | `random(s_idx, t_idx, domain=:spacetime, model=(besag, ar1))` | `random(s_idx, model=besag) ⊗ random(t_idx, model=ar1)` |

The `random(domain=:spacetime)` module serves as a convenient shorthand. The framework determines the interaction type by checking if the provided spatial and temporal models are structured (e.g., `besag`, `ar1`) or unstructured (`iid`).
