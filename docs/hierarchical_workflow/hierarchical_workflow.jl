# ==============================================================================
# BSTM INTEGRATED HIERARCHICAL SPATIOTEMPORAL & ADR TELEMETRY WORKFLOW
# ==============================================================================
# Multi-Tier Ecological Pipeline with Tier-Specific Spatial Networks,
# Continuous Surface Derivatives, Errors-in-Variables (EIV) Priors,
# Full Monte Carlo Matrix Resharding (U_dest = P * U_src),
# Second-Last Tier Habitat Suitability Index (HSI) Determination,
# Lagrangian Mark-Recapture Telemetry, and DuckDB Relational Persistence.
# ==============================================================================
...
println("="^80)
println("STARTING INTEGRATED HIERARCHICAL & ADR TELEMETRY WORKFLOW")
println("="^80)

# ------------------------------------------------------------------------------
# STEP 0: SYNTHETIC DATA GENERATION ACROSS MULTI-TIER ECOSYSTEM
# ------------------------------------------------------------------------------
...
# Generate mark-recapture telemetry simulation bundle via bstm_data
domain_km = 600.0
n_marks = 100
...
# ------------------------------------------------------------------------------
# STEP 2: TIER 1 - CONTINUOUS BATHYMETRY & ANALYTICAL SURFACE DERIVATIVES
# ------------------------------------------------------------------------------
...
# Surface derivatives at Tier 2 grab locations
...
# Surface derivatives at Master Mesh centroids
...
@info "Tier 1 Bathymetry complete with surface derivatives."

# ------------------------------------------------------------------------------
# STEP 3: TIER 2 - SEABED SUBSTRATE ON NETWORK G_2 WITH EIV
# ------------------------------------------------------------------------------
@info "Step 3: Fitting Tier 2 Substrate on Network G_2 (25 units) with EIV..."

pred_sub_pts = bstm_surface_derivatives(
    m_depth, chn_depth, df_sub[:, [:s_x, :s_y]]; metrics=[:slope]
)
...
sub_means_master = resharded_sub_master.mean
sub_sds_master   = resharded_sub_master.std

tier2_table = DataFrame(
    unit_id = 1:au_master.n_units,
    pred_mean = sub_means_master,
    pred_sd = sub_sds_master,
...
@info "Tier 2 Substrate complete (Full Monte Carlo Matrix resharded)."

# ------------------------------------------------------------------------------
# STEP 4: TIER 3 - OCEANOGRAPHIC TEMPERATURE ON NETWORK G_3 WITH EIV
# ------------------------------------------------------------------------------
@info "Step 4: Fitting Tier 3 Oceanography on Network G_3 (40 units) (BYM2 ⊗ AR1)..."
...
# Reshard from G_3 to G_4 and G_master
...
temp_means_master = resharded_temp_master.mean
temp_sds_master   = resharded_temp_master.std

tier3_table = DataFrame(
    unit_id = 1:au_master.n_units,
    year = fill(2024, au_master.n_units),
    pred_mean = temp_means_master,
    pred_sd = temp_sds_master,
...
@info "Tier 3 Oceanography complete and resharded."

# ------------------------------------------------------------------------------
# STEP 5: TIER 4 - COMMUNITY PCA & HABITAT SUITABILITY (HSI) ON NETWORK G_4
# ------------------------------------------------------------------------------
# In this second-last tier, we simultaneously determine Habitat Suitability (HSI)
# and community ordination gradients (PC1, PC2) on the haul network G_4 (50 units).
#
# Primary HSI Formulation:
#   Binomial / Bernoulli model where observed survey densities below
#   a threshold quantile (default p = 0.05) are classified as poor habitat (0),
#   and densities >= threshold are classified as suitable habitat (1).
# ------------------------------------------------------------------------------
@info "Step 5: Tier 4 Community Ordination & Habitat Suitability Modeling on G_4..."

haul_ids      = unique(df_species_comp.haul_id)
species_names = unique(df_species_comp.species)
N_hauls = length(haul_ids)
K_spp   = length(species_names)

haul_dict = Dict(h => i for (i, h) in enumerate(haul_ids))
spp_dict  = Dict(s => j for (j, s) in enumerate(species_names))

Y_mat = zeros(Float64, N_hauls, K_spp)
haul_meta = DataFrame(
    haul_idx = 1:N_hauls,
    haul_id  = haul_ids,
    year     = zeros(Int, N_hauls),
    s_x      = zeros(Float64, N_hauls),
    s_y      = zeros(Float64, N_hauls)
)
...
# 1. Derive Habitat Suitability Index (HSI) on G_4 via Quantile Threshold (p = 0.05)
q_threshold_comm = quantile(haul_meta.total_density, 0.05)
haul_meta.habitat_suitable = Int.(haul_meta.total_density .>= q_threshold_comm)
@info @sprintf("  ✓ Tier 4 Habitat threshold quantile (p=0.05): %.3f kg/km² (Poor: %d, Suitable: %d)",
               q_threshold_comm,
               count(==(0), haul_meta.habitat_suitable),
               count(==(1), haul_meta.habitat_suitable))

# Alternative A: Continuous Soft Sigmoidal Suitability Score
iqr_comm = max(1e-4, quantile(haul_meta.total_density, 0.75) - quantile(haul_meta.total_density, 0.25))
haul_meta.hsi_continuous = 1.0 ./ (1.0 .+ exp.(-2.5 .* (haul_meta.total_density .- q_threshold_comm) ./ iqr_comm))

# Hellinger Transformation & Eigen-Decomposition for Community PC1 and PC2
row_sums = sum(Y_mat, dims=2)
row_sums[row_sums .== 0.0] .= 1.0
H_mat = sqrt.(Y_mat ./ row_sums)
H_centered = H_mat .- mean(H_mat, dims=1)
Cov_H = (H_centered' * H_centered) ./ (N_hauls - 1)
eigen_decomp = eigen(Symmetric(Cov_H))

evals = reverse(eigen_decomp.values)
evecs = reverse(eigen_decomp.vectors, dims=2)
var_explained = evals ./ sum(evals) .* 100.0
@info @sprintf("  ✓ Community Eigen-reduction: PC1 explains %.1f%%, PC2 explains %.1f%% variance",
               var_explained[1], var_explained[2])

haul_scores = H_centered * evecs[:, 1:2]
haul_meta.PC1 = haul_scores[:, 1]
haul_meta.PC2 = haul_scores[:, 2]
haul_meta.year_idx = haul_meta.year .- (minimum(haul_meta.year) - 1)

# Attach resharded Substrate and Temperature on Network G_4 with EIV standard errors
...
# Model A: Habitat Suitability Model (HSI) on Network G_4 (Bernoulli)
m_hsi = @bstm(
...
# Model B: Community PC1 on Network G_4
m_pc1 = @bstm(
...
# Model C: Community PC2 on Network G_4
m_pc2 = @bstm(
...
# Predict HSI, PC1, and PC2 on Network G_4 units (Year 10)
...
# Reshard HSI, PC1, PC2 from G_4 to Master Network G_master
resharded_hsi_master = reshard_spatial_field(
    pred_hsi_u4.predictions_denoised, au_comm, au_master; mode=:samples
)
hsi_means_master = 1.0 ./ (1.0 .+ exp.(-resharded_hsi_master.mean))
hsi_sds_master   = (1.0 ./ (1.0 .+ exp.(-resharded_hsi_master.upper)) .- 
                    1.0 ./ (1.0 .+ exp.(-resharded_hsi_master.lower))) ./ 3.92

resharded_pc1_master = reshard_spatial_field(
    pred_pc1_u4.predictions_denoised, au_comm, au_master; mode=:samples
)
pc1_means_master = resharded_pc1_master.mean
pc1_sds_master   = resharded_pc1_master.std

resharded_pc2_master = reshard_spatial_field(
    pred_pc2_u4.predictions_denoised, au_comm, au_master; mode=:samples
)
pc2_means_master = resharded_pc2_master.mean
pc2_sds_master   = resharded_pc2_master.std

community_pred_df = DataFrame(
    unit_id  = 1:au_master.n_units,
    hsi_mean = hsi_means_master,
    hsi_sd   = hsi_sds_master,
    pc1_mean = pc1_means_master,
    pc1_sd   = pc1_sds_master,
    pc2_mean = pc2_means_master,
    pc2_sd   = pc2_sds_master
)
...
@info "Tier 4 Community PCA & Habitat Suitability complete and resharded to G_master."

# ------------------------------------------------------------------------------
# STEP 6: TIER 5 - INTEGRATED SNOW CRAB BIOMASS & ADR MOVEMENT ON G_master
# ------------------------------------------------------------------------------
@info "Step 6: Fitting Tier 5 Snow Crab Biomass on Master Network G_master (80 units)..."

df_crab.year_idx = df_crab.year .- (minimum(df_crab.year) - 1)
df_crab.log_effort = log.(df_crab.swept_area_km2)

# Attach master-harmonized environmental, community, and HSI covariates with EIV uncertainties
...
@info "Tier 5 Snow Crab model complete and saved on Master Network."

# ------------------------------------------------------------------------------
# STEP 7: DISPERSAL TRAJECTORIES, REGIONAL CONNECTIVITY & VISUALIZATIONS
# ------------------------------------------------------------------------------
@info "Step 7: Generating Dispersal Trajectories, Connectivity, and Master Visualizations..."

# Derived Advective Velocity & Transition Kernel
...
# Simulate 25 individual movement paths (CRW with persistence)
...
# Macro-regional connectivity across East/West domain boundary
...
# Visualizations: 4-Panel Publication Dashboard
p_hsi = choropleth(au_master.polygons, community_pred_df.hsi_mean; title="Tier 4 Derived HSI Surface", colormap=:viridis)
p_biomass = choropleth(au_master.polygons, crab_pred_df.pred_mean; title="Tier 5 Snow Crab Biomass", colormap=:plasma)

p_traj = plot(title="Dispersal Trajectories (CRW Persistence)", aspect_ratio=:equal)
...
savefig(p_master_dashboard, joinpath(output_dir, "hierarchical_workflow_dashboard.png"))
@info "  ✓ Publication dashboard saved to $(joinpath(output_dir, "hierarchical_workflow_dashboard.png"))"

# ------------------------------------------------------------------------------
# STEP 8: MASTER HARMONIZATION TABLE & ZERO-COPY SQL HABITAT QUERIES
# ------------------------------------------------------------------------------
...
println("\n" * "="^80)
println("INTEGRATED HIERARCHICAL & ADR WORKFLOW COMPLETED SUCCESSFULLY!")
println("="^80)
