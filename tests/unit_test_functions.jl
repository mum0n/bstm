...
@testset "BSTM Full Test Suite" begin
    ...
    @testset "End-to-End Smoke Tests" begin
        ...
        # Test simple BYM2+AR1 model
        @testset "Simple BYM2+AR1" begin
            model = @bstm(likelihood(y, family=:gaussian) ~ 1 + x_cont + spatial(s_idx, model="bym2") + temporal(t_idx, model="ar1"), dummy_df, W=W_test)
        ...
        @testset "Multivariate Smoke Test" begin
            ...
            model = @bstm(likelihood(y, family=:gaussian) + likelihood(y2, family=:gaussian) ~ 1 + x_cont + spatial(s_idx, model="icar"), dummy_df, W=W_test)
            chain = sample(model, MH(), 10; progress=false)
            @test chain isa Chains
        end
    end
