    ...
    function init_params_merge( source_chain; target_chain=NaN, load_from_file=false, overrides::Union{Dict, Nothing}=nothing, fn_inits = "init_params.jl2"  )
        """BSTM Utility Function v2.1.0
        Timestamp: 2026-07-08
        Synopsis: Merges parameter mean values from a source MCMC chain summary (`source_chain`)
                with optional updates from `target_chain` and explicit `overrides`. This is useful
                for initializing a new MCMC run by combining parameters from different previous runs
                or applying specific adjustments.

        Inputs:
            - source_chain: The MCMC chain object providing the base set of parameters.
            - target_chain: (Optional) An MCMC chain object whose parameters will update `source_chain`'s
                            parameters if they overlap. Defaults to NaN.
        ...
        # Start with parameters from source_chain
        ressumm_base = summarize(source_chain)
        vns_final = ressumm_base.nt.parameters
        means_final = deepcopy(ressumm_base.nt[2])

        # If target_chain is provided and not NaN, it updates parameters from source_chain
        if !(target_chain isa Number && isnan(target_chain))
            ressumm_update = summarize(target_chain)
            vns_update = ressumm_update.nt.parameters
            means_update = ressumm_update.nt[2]

            for (idx_update, vn_update) in enumerate(vns_update)
                idx_final = findfirst(x -> x == vn_update, vns_final)
                if !isnothing(idx_final)
                    # Update existing parameter
                    means_final[idx_final] = means_update[idx_update]
                else
                    # Add new parameter if it doesn't exist in the base
                    push!(vns_final, vn_update)
                    push!(means_final, means_update[idx_update])
                end
            end
        end
        ...
        if !isnothing(overrides)
            for (pattern, values) in overrides
                u = findall(x -> occursin(Regex(pattern), String(x)), vns_final)
                if !isempty(u)
                    if length(u) == length(values)
                        means_final[u] .= values
                    else
        ...
        init_params = FillArrays.Fill( means_final )
    ...
    function init_params( res=NaN; load_from_file=false, overrides::Union{Dict, Nothing}=nothing, fn_inits = "init_params.jl2"  )
        """BSTM Utility Function v2.0.0
        Timestamp: 2026-07-08
        Synopsis: Extracts mean parameter values from a Turing MCMC chain summary to be used as initial
                ...
                custom overrides. This function consolidates the functionality of the previous
                `init_params_extract` and `init_params_copy` functions.
    ...
    function _generate_model_pseudocode(m::DynamicPPL.Model)
        ...
        if family in ["gaussian", "lognormal", "student_t", "laplace", "half_normal", "half_student_t"] && !get(config, :use_sv, false)
            push!(lines, "    y_sigma ~ Exponential(1.0)") # Observation noise standard deviation
        end
...
function _generate_model_pseudocode(m::DynamicPPL.Model)
    ...
    # Global priors
    family = haskey(config, :likelihood_specs) && !isempty(config.likelihood_specs) ? string(get(config.likelihood_specs[1], :family, "gaussian")) : "gaussian"
    if family == "negbin"
        push!(lines, "    r_nb ~ Exponential(1.0)")
    end
    if get(config, :use_zi, false)
    ...
    if get(config, :Xfixed_N, 0) > 0
        push!(lines, "\n    # Prior for fixed effects")
        if haskey(config, :Xfixed_names) && !isempty(config.Xfixed_names)
            push!(lines, "    Xfixed_beta ~ MvNormal(zeros($(get(config, :Xfixed_N, 0))), 5.0 * I) # Coefficients for: $(join(config.Xfixed_names, ", "))")
        else
            push!(lines, "    Xfixed_beta ~ MvNormal(zeros($(get(config, :Xfixed_N, 0))), 5.0 * I)")
        end
    end
    ...
    if get(config, :Xfixed_N, 0) > 0 # Fixed effects contribution
        if haskey(config, :Xfixed_names) && !isempty(config.Xfixed_names)
            push!(eta_parts, "M.Xfixed * Xfixed_beta # Fixed effects: $(join(config.Xfixed_names, ", "))")
        else
            push!(eta_parts, "M.Xfixed * Xfixed_beta")
        end
    end
...
function show_model(m::DynamicPPL.Model)
    ...
    family_str = haskey(config, :likelihood_specs) && !isempty(config.likelihood_specs) ? string(get(config.likelihood_specs[1], :family, "N/A")) : "N/A"

    # Use `get` with a fallback to `nameof(m.f)` for the model name.
    println("Model Name: ", get(config, :model_name, nameof(m.f)))
    println("Model Architecture: ", get(config, :model_arch, "N/A"))
    println("Model Family: ", family_str)
    println("Number of observed data points: ", get(config, :y_N, "N/A"))
    println("Number of spatial units: ", get(config, :N_areas, get(config, :s_N, "N/A")))
    println("Number of time units: ", get(config, :N_time, get(config, :t_N, "N/A")))
    println("Number of covariates: ", get(config, :N_cov, "N/A"))
    ...
    @model function bstm_univariate(M, ::Type{T}=Float64) where {T}
        ...
        # #
        # Fixed Effects Realization
        if M.Xfixed_N > 0
            # This logic samples each fixed effect coefficient individually, allowing for
            # mixed prior types (e.g., Normal, Exponential) which is not supported by
            # the `Product` distribution constructor in some versions of Distributions.jl.
            Xfixed_beta = Vector{T}(undef, M.Xfixed_N)
            priors = get(M, :Xfixed_priors_vec, [Normal(0, 5) for _ in 1:M.Xfixed_N])
            for i in 1:M.Xfixed_N
                Xfixed_beta[i] ~ NamedDist(priors[i], Symbol("Xfixed_beta[", i, "]"))
            end
            eta .+= vec(M.Xfixed * Xfixed_beta)
        end
        ...
        for spec in M.manifolds
            ...
            if m_obj isa IID
                ...
                field_raw ~ NamedDist(filldist(Normal(0, sigma_val), n_units), Symbol("latent_", key))
                field = vec(field_raw)
                eta .+= field[indices]
            ...
            elseif m_obj isa Union{ICAR, Besag, RW1, RW2, Leroux, Cyclic}
                ...
                field_raw ~ NamedDist(MvNormalCanon(zeros(n_units), Q), Symbol("latent_", key))
                field = vec(field_raw)
                if !(m_obj isa Cyclic)
            ...
            elseif m_obj isa SAR
                ...
                field_raw ~ NamedDist(MvNormalCanon(zeros(n_units), Q_final), Symbol("latent_", key))
                field = vec(field_raw)
            ...
            elseif m_obj isa Union{PSpline, BSpline, TPS, RFF, FFT, Wavelet, Moran, Spherical, ExponentialDecay, Barycentric}
                ...
                if m_obj isa RFF
                    ...
                    if m_obj isa PSpline || m_obj isa TPS
                        ...
                        latent_coeffs_raw ~ NamedDist(MvNormalCanon(zeros(n_basis_cols), Q_penalty), beta_name)
                        latent_coeffs = vec(latent_coeffs_raw)
                        Turing.@addlogprob! logpdf(Normal(0, 0.001 * n_basis_cols), sum(latent_coeffs))
                    else
                        latent_coeffs_raw ~ NamedDist(filldist(Normal(0, sigma_val), n_basis_cols), beta_name)
                        latent_coeffs = vec(latent_coeffs_raw)
                    end
                    eta .+= B_mat * latent_coeffs
                end

            elseif m_obj isa Union{GP, FITC, SVGP, Nystrom}
                # --- Sparse Gaussian Process Manifolds (FITC/Nystrom/SVGP) ---
                # This block handles various sparse GP approximations using the
                # FITC (Fully Independent Training Conditional) formulation.
                # For scalability, even a standard 'gp' is treated as a sparse GP.
                ...
                # Define the kernel
                kernel = get_kernel_from_string(m_obj.kernel)
                kernel_scaled = sigma_val^2 * (kernel ∘ ScaleTransform(1.0 / ls_val))

                # Inducing points are pre-computed in bstm_config and passed in the spec
                ...
                # Non-centered parameterization for latent values at inducing points
                L_uu = cholesky(Symmetric(K_uu)).L
                u_raw_mat ~ NamedDist(MvNormal(zeros(n_inducing), I), Symbol("u_raw_", key))
                u_raw = vec(u_raw_mat)
                u_latent = L_uu * u_raw

                # FITC approximation for the conditional distribution of f
                A = L_uu' \ K_uf
                mean_f = A' * u_latent
                var_f = k_ff_diag - vec(sum(A.^2, dims=1))

                # Add the latent GP field to the linear predictor using a non-centered parameterization
                f_innov_mat ~ NamedDist(MvNormal(zeros(n_obs), I), Symbol("f_innov_", key))
                f_innov = vec(f_innov_mat)
                eta .+= mean_f + sqrt.(max.(var_f, 0.0) .+ noise) .* f_innov
            ...
            elseif m_obj isa SVCManifold
                ...
                if inner_m isa BYM2
                    ...
                    s_struct_raw ~ NamedDist(MvNormalCanon(zeros(M.s_N), spec.Q_template + noise * I), Symbol("svc_struct_", key))
                    s_unstruct_raw ~ NamedDist(MvNormal(zeros(M.s_N), I), Symbol("svc_iid_", key))
                    s_struct = vec(s_struct_raw)
                    s_unstruct = vec(s_unstruct_raw)
                    beta_svc = sig_svc .* (sqrt(rho_svc) .* s_struct .+ sqrt(1.0 - rho_svc) .* s_unstruct)
                    eta .+= beta_svc[M.s_idx] .* x_svc
                else
                    beta_svc_raw ~ NamedDist(filldist(Normal(0, sig_svc), M.s_N), Symbol("beta_svc_", key))
                    beta_svc = vec(beta_svc_raw)
                    eta .+= beta_svc[M.s_idx] .* x_svc
                end
            ...
            elseif m_obj isa ComposedManifold
                ...
                if op == :kronecker_product || op == :composition
                    ...
                    field_raw ~ NamedDist(MvNormalCanon(zeros(n_units), Symmetric(Q_final)), Symbol("latent_", key))
                    field = vec(field_raw)
                ...
                elseif op == :direct_sum
                    ...
                    field_raw ~ NamedDist(MvNormalCanon(zeros(n_total), Symmetric(Q_final)), Symbol("latent_", key))
                    field = vec(field_raw)
    ...
    @model function bstm_multivariate(M, ::Type{T}=Float64) where {T}
        ...
        for spec in M.manifolds
            ...
            for k in 1:outcomes_N
                ...
                s_icar_raw ~ NamedDist(MvNormalCanon(zeros(M.s_N), spec.Q_template + noise * I), struct_name)
                s_iid_raw ~ NamedDist(MvNormal(zeros(M.s_N), I), iid_name)
                s_icar = vec(s_icar_raw)
                s_iid = vec(s_iid_raw)
                    Turing.@addlogprob! logpdf(Normal(0, 0.001 * M.s_N), sum(s_icar))
                    combined = sigma_val .* (sqrt(rho_val) .* s_icar .+ sqrt(1.0 - rho_val) .* s_iid)
                ...
                # Ensure innovations are treated as a vector to prevent dimension mismatch.
                innovations_raw ~ NamedDist(MvNormal(zeros(M.t_N), I), innov_name)
                innovations = vec(innovations_raw)
                ...
                elseif m_obj isa ComposedManifold
                    ...
                    elseif op == :pipe
                        ...
                        if dynamic_manifold isa AR1
                            ...
                            n_state = size(Q_innov, 1)
                            pipe_field = Matrix{T}(undef, n_state, M.t_N)
                            innov_base_raw ~ NamedDist(MvNormal(zeros(n_state), I), Symbol("innov_base_", key, "_1"))
                            innov_base = vec(innov_base_raw)
                            pipe_field[:, 1] = (L_innov \ innov_base) ./ sqrt(1.0 - rho_pipe^2 + noise)
                            for t in 2:M.t_N
                                innov_t_raw ~ NamedDist(MvNormal(zeros(n_state), I), Symbol("innov_base_", key, "_", t))
                                innov_t = vec(innov_t_raw)
                                pipe_field[:, t] = rho_pipe .* pipe_field[:, t-1] .+ (L_innov \ innov_t)
                            end
                        ...
                        elseif dynamic_manifold isa Union{RFF, TPS, PSpline, BSpline, FFT, Wavelet, Spherical, ExponentialDecay, Barycentric}
                            ...
                            L_coeffs = cholesky(Symmetric(Q_coeffs)).L
                            for j in 1:n_basis
                                innov_j_raw ~ NamedDist(MvNormal(zeros(n_state), I), Symbol("coeffs_base_", key, "_", j))
                                innov_j = vec(innov_j_raw)
                                coeffs_field[:, j] = L_coeffs \ innov_j
                            end
    ...
    @model function bstm_multifidelity(M, ::Type{T}=Float64) where {T}
        ...
        if haskey(M, :z_coords_s) && haskey(M, :z_obs)
            z_sigma ~ NamedDist(get(M.hyperpriors, "z_sigma_prior", Exponential(0.5)), :z_sigma)
