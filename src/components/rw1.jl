...
# --- Implementation of the Explicit Interface for RW1 ---

"""
    RW1 <: ComponentModel

v1.9.1 (2026-08-07) - The First-Order Random Walk (RW1) model.
This model assumes that the current value of a latent field is the previous value
plus a random innovation. It is an intrinsic Gaussian Markov Random Field (GMRF)
with a rank deficiency of 1, implying a sum-to-zero constraint for identifiability.
...
COMPONENT_CONSTRUCTORS[:rw1] = (p, params) -> RW1(p.sigma)
...
v1.9.1 (2026-08-07) - Data-dependent setup for the RW1 component.
...
v1.9.1 (2026-08-07) - Pre-computes the structure matrix for the RW1 component.
For an RW1 model, the precision matrix template defines the first-order differences.
...
function get_precomputes(m::RW1, M::NamedTuple, mod_data::Dict)::NamedTuple
    t_N = get(M, :t_N, 0)
    if t_N == 0
        @warn "Could not determine number of time steps for RW1 component '$(mod_data[:key])'. The component will have no effect."
    end
...
"""
    get_priors(m::RW1, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String

v1.9.1 (2026-08-07) - Generates the Turing code for the RW1 component's priors.
...
"""
function get_priors(m::RW1, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String
...
"""
    get_updates(m::RW1, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String

v1.9.1 (2026-08-07) - Generates the Turing code for the RW1 component's update logic using spectral sampling.
This function implements the AD-safe spectral sampling method for the RW1 model. It generates
code that constructs the latent field by transforming standard normal innovations using the
pre-computed spectral decomposition (`U`, `L`) of the RW1 precision matrix, avoiding any
`cholesky` calls on parameter-dependent matrices. It also enforces the sum-to-zero constraint.
"""
function get_updates(m::RW1, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String
    v = generate_full_variable_names(spec, arch, outcome_idx)
    ...
    return """
    # --- RW1 Spectral Assembly: $(spec.key) ---
    # This block constructs the RW1 temporal effect using a non-centered parameterization
    # based on the spectral decomposition of the RW1 precision matrix. This method is
    ...
    #    RW1 has a rank deficiency of 1, so the first eigenvalue is zero.
    diag_D[1] = zero(T)
...
"""
    get_effects(m::RW1, chain, M::NamedTuple, n_samples::Int, outcomes_N::Int, p_names::NamedTuple, spec::NamedTuple, PS::Union{NamedTuple, Nothing}, N_total::Int)::NamedTuple

v1.9.1 (2026-08-07) - Reconstructs the RW1 component's effect from posterior samples.
This function extracts the posterior samples for `sigma` and `struct` from the MCMC
chain. It then reconstructs the full posterior distribution of the latent RW1 field by
re-running the spectral transformation for each posterior sample.
"""
function get_effects(m::RW1, chain, M::NamedTuple, n_samples::Int, outcomes_N::Int, p_names::NamedTuple, spec::NamedTuple, PS::Union{NamedTuple, Nothing}, N_total::Int)::NamedTuple
    structured_effects = []
