...
A component model for a Log-Gamma Cox Process (LGCP) with a Negative Binomial observation model.
This is used for modeling spatial or spatiotemporal point patterns where the intensity
function is a Gamma Process, and aggregated counts in discrete areal units follow
a Negative Binomial distribution.

# Fields
- `model::ComponentModel`: The inner model (e.g., `ICAR`, `Leroux`) that defines the
  structure of the latent Gamma Process.
...
COMPONENT_CONSTRUCTORS[:lgammap] = (p, params) -> begin
    inner_model_obj = get(params, :inner_model_obj, error("LogGammaCoxProcess constructor requires an `inner_model_obj`."))
...
"""
    get_datastructures!(m_type::Type{<:LogGammaCoxProcess}, M::Dict, mod_data::Dict)::Bool

Performs data-dependent setup for the `LogGammaCoxProcess` component. It delegates spatial context
setup to its inner model, processes `grid_areas`, and reshapes the observation data
into a `[space x time]` matrix.
"""
function get_datastructures!(m_type::Type{<:LogGammaCoxProcess}, M::Dict, mod_data::Dict)::Bool
    params = mod_data[:params]

    # Delegate data structure setup to the inner model
    inner_model_spec_node = get(params, :inner_model_node, error("LogGammaCoxProcess model requires an `inner_model_node` parameter."))
    ...
    # The inner model is always spatial for LogGammaCoxProcess
    ...
    if haskey(params, :grid_areas)
        ...
        elseif ga_val isa AbstractVector
            ...
            catch; @warn "Could not resolve grid_areas for LogGammaCoxProcess. Defaulting to unit areas."; M[:grid_areas] = ones(M[:s_N]); end
        end
    else
        @warn "LogGammaCoxProcess model specified, but `grid_areas` parameter is missing. Defaulting to unit areas."
        M[:grid_areas] = ones(get(M, :s_N, 1))
    end

    # Reshape the observation vector into a matrix [s_N x t_N]
    if M[:outcomes_N] > 1
        error("LogGammaCoxProcess currently only supports a single outcome variable representing counts.")
    end
...
Performs pre-calculations for the `LogGammaCoxProcess` component. It delegates to its inner model
to get the precision matrix structure and determines if a spatiotemporal model is being used.
"""
function get_precomputes(m::LogGammaCoxProcess, M::NamedTuple, mod_data::Dict)::NamedTuple
    params = mod_data[:params]
...
"""
    get_priors(m::LogGammaCoxProcess, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String

Generates the Turing code for the `LogGammaCoxProcess` component's priors.
"""
function get_priors(m::LogGammaCoxProcess, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String
    p_names = generate_full_variable_names(spec, arch, outcome_idx)
    
    priors = ["$(p_names.shape) ~ NamedDist($(_distribution_to_string(m.shape)), :$(p_names.shape))"]
...
"""
    get_updates(m::LogGammaCoxProcess, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String

Generates the Turing code to construct the `LogGammaCoxProcess` effect and its custom likelihood.
This component overrides the standard likelihood evaluation.
"""
function get_updates(m::LogGammaCoxProcess, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String
    p_names = generate_full_variable_names(spec, arch, outcome_idx)
    ...
    return """
        ...
        local gamma_shape = $(p_names.shape) # The learned shape parameter
...
"""
    get_effects(m::LogGammaCoxProcess, chain, M::NamedTuple, n_samples::Int, outcomes_N::Int, p_names::NamedTuple, spec::NamedTuple, PS::Union{NamedTuple, Nothing}, N_total::Int)::NamedTuple

Reconstructs the `LogGammaCoxProcess` component's mean intensity surface from posterior samples.
"""
function get_effects(m::LogGammaCoxProcess, chain, M::NamedTuple, n_samples::Int, outcomes_N::Int, p_names::NamedTuple, spec::NamedTuple, PS::Union{NamedTuple, Nothing}, N_total::Int)::NamedTuple
    shape_samples = get(chain, p_names.shape)
    ...
    mean_intensity_surfaces = zeros(n_samples, s_N, t_N)
    ...
    # For a simplified effect, we reconstruct only the Gamma Process part of the intensity.
    ...
    return (
        mean_intensity_gp_part=(mean=mean_effect, lower=lower_ci, upper=upper_ci),
    )
end
