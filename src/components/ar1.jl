...
function get_priors(
    ...
    if !is_multivariate || (is_multivariate && (!is_shared || is_first_outcome))
        ...
        unconstrained_rho_name = "unconstrained_rho_$(spec.key)"
        if is_multivariate && !is_shared
            unconstrained_rho_name *= "_$(outcome_idx)"
        end
        push!(priors_acc, "$(unconstrained_rho_name) ~ $(_distribution_to_string(m.unconstrained_rho))")
    end
...
function get_updates(
    ...
    statespace_code = """
        ...
        rho = tanh($(unconstrained_rho_name))
        latent_field = ar1_statespace(rho, $(p_names.sigma), $(p_names.innovations), $(n_latent), M.noise)
    ...
    centered_code = """
        ...
        rho = tanh($(unconstrained_rho_name))
        let
            K = _ar1_covariance_matrix(rho, $(p_names.sigma), $(n_latent), M.noise)
    ...
    elseif m.method == :centered
        ...
        error("Unsupported method '$(m.method)' for AR1 component. Use `:statespace`, `:spectral`, or `:centered`.")
    end
end
...
function get_effects(
    ...
    for k in 1:outcomes_N
        ...
        unconstrained_rho_name = _find_parameter(p_names_vec, string(spec.key), "unconstrained_rho", k, is_multivariate_model)
        ...
        if isempty(sigma_name) || isempty(unconstrained_rho_name) || 
           (m.method in [:statespace, :spectral] && isempty(innovations_name)) ||
           (m.method == :centered && isempty(latent_name))
            @warn "Parameters for AR1 component $(spec.key) (outcome $k, method $(m.method)) not found. Returning zero-matrix."
            push!(structured_effects, zeros(Float64, N_total, n_samples))
            continue
        end

        sigma_samples = get_params_vector(chain, sigma_name, 1)[:, 1]
        unconstrained_rho_samples = get_params_vector(chain, unconstrained_rho_name, 1)[:, 1]
        rho_samples = tanh.(unconstrained_rho_samples)
        ...
        if m.method in [:statespace, :spectral]
            ...
            for j in 1:n_samples
                ...
                if m.method == :statespace
                    latent_field_train = ar1_statespace(rho_samples[j], sigma_samples[j], innovations_samples_train[j, :], M.t_N, noise_val)
                else # :spectral
                    U = spec.hyper.U
                    L_base = spec.hyper.L
                    lambda_vals = (1.0 + rho_samples[j]^2) .+ rho_samples[j] .* L_base
                    diag_D = sigma_samples[j] ./ sqrt.(lambda_vals .+ M.noise)
                    latent_field_train = U * (diag_D .* innovations_samples_train[j, :])
                end
        ...
        if t_N_full > M.t_N
            for j in 1:n_samples
                for t in (M.t_N + 1):t_N_full
                    latent_field_samples[t, j] = rho_samples[j] * latent_field_samples[t-1, j] + randn() * sigma_samples[j]
                end
            end
        end
