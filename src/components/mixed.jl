...
COMPONENT_CONSTRUCTORS[:mixed] = (p, params) -> begin
    ...
    inner_model_obj = get(params, :inner_model_obj, error("Mixed requires an `inner_model_obj`."))
...
"""
    get_priors(m::Mixed, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String

Generates priors for the `Mixed`, handling both simple and correlated effects.
"""
function get_priors(m::Mixed, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String
    ...
    if n_terms == 1
        ...
        $(p_names.L_corr) ~ NamedDist(LKJCholesky($(n_terms), T(1.0)), :$(p_names.L_corr))
        $(p_names.sigma_effects) ~ NamedDist(filldist(Exponential(T(1.0)), $(n_terms)), :$(p_names.sigma_effects))
        $(p_names.raw) ~ NamedDist(MvNormal(zeros(T, $(n_groups * n_terms)), I), :$(p_names.raw))
        """
    end
end

"""
    get_updates(m::Mixed, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String

Generates the Turing code for constructing and applying the mixed effect.
"""
function get_updates(m::Mixed, spec::NamedTuple, arch::String, outcome_idx::Union{Int, Nothing}, M::NamedTuple)::String
    p_names = generate_full_variable_names(spec, arch, outcome_idx)
    ...
    if n_terms == 1
        ...
        # This is a bit brittle but necessary for delegation.
        update_inner_cleaned = replace(inner_update_frags, Regex("M\\.\\w+_idx") => "M.$(index_var)")
        ...
        if lhs_str == "1" || lhs_str == "intercept()"
            ...
            update_without_eta = replace(update_inner_cleaned, Regex("$(eta_target) \\.\\+= .*") => "")
            return """
            $(update_without_eta)
        ...
        for i in 1:n_terms
            ...
            if term == "1" || term == "intercept()"
                ...
                application_loop *= "local cov_data_$(i) = T.(M.data[!, :$(Symbol(term))]); for j in 1:length($(eta_target)); $(eta_target)[j] += cov_data_$(i)[j] * effects_matrix[M.$(index_var)[j], $(i)]; end\n"
            end
        end
        ...
        local gamma_matrix::Matrix{T} = L_groups_cov_inv_t \\ innovations_matrix
        local effects_matrix::Matrix{T} = gamma_matrix * L_effects_t
...
"""
    get_effects(m::Mixed, chain, M::NamedTuple, n_samples::Int, outcomes_N::Int, spec::NamedTuple, PS::Union{NamedTuple, Nothing}, N_total::Int)::NamedTuple
...
"""
function get_effects(m::Mixed, chain, M::NamedTuple, n_samples::Int, outcomes_N::Int, spec::NamedTuple, PS::Union{NamedTuple, Nothing}, N_total::Int)::NamedTuple
    n_terms = length(m.lhs)
    n_groups = spec.hyper.inner_precomputes.n_latent
    noise = M.noise
    ...
    if n_terms == 1
        ...
        for k in 1:outcomes_N
            ...
            raw_samples = get_params_vector(chain, string(p_names.raw), n_groups * n_terms)
            
            F_groups = cholesky(Symmetric(Matrix(spec.hyper.inner_precomputes.Q_template) + noise * I))
